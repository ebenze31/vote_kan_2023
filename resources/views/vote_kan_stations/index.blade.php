                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                <thead>
                                    <tr>
                                        <th>#</th><th>Name</th><th>Phone</th><th>Phone 2</th><th>Province</th><th>Amphoe</th><th>Area</th><th>Tambon</th><th>Polling Station At</th><th>User Id</th><th>Name User</th><th>Amount Add Score</th><th>Quantity Person</th><th>Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                @foreach($vote_kan_stations as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->name }}</td><td>{{ $item->phone }}</td><td>{{ $item->phone_2 }}</td><td>{{ $item->province }}</td><td>{{ $item->amphoe }}</td><td>{{ $item->area }}</td><td>{{ $item->tambon }}</td><td>{{ $item->polling_station_at }}</td><td>{{ $item->user_id }}</td><td>{{ $item->name_user }}</td><td>{{ $item->amount_add_score }}</td><td>{{ $item->quantity_person }}</td>
                                        <td>
